...
function PublisherPanel({ sharedImage, sharedCaption, sharedTags, onToast }) {
  ...
  const [platforms, setPlatforms] = useState({ fb: true, ig: true });
  const [automationMode, setAutomationMode] = useState("manual");
  const [metaAccounts, setMetaAccounts] = useState(() => {
    const saved = localStorage.getItem('infiCreatorUser');
    if (saved) {
      const data = JSON.parse(saved);
      return data.linkedAccounts?.filter(a => a.provider === 'meta') || [];
    }
    return [];
  });
  const [selectedAccount, setSelectedAccount] = useState(metaAccounts[0]?.id || "");
  ...
  const addManualAccount = () => {
    if (!manualPageId || !manualToken) {
      onToast("❌ Please fill in both Page ID and Access Token");
      return;
    }
    const newAccount = {
      id: manualPageId,
    ...
    setSelectedAccount(manualPageId);
    setManualPageId("");
    setManualToken("");
    ...
    if (saved) {
      ...
      if (!data.linkedAccounts.find(a => a.id === manualPageId)) {
        data.linkedAccounts.push(newAccount);
        localStorage.setItem('infiCreatorUser', JSON.stringify(data));
      }
    }
    onToast("✅ Meta account added successfully!");
  };

  const removeAccount = (accountId) => {
    setMetaAccounts(metaAccounts.filter(a => a.id !== accountId));
    if (selectedAccount === accountId) setSelectedAccount(metaAccounts[0]?.id || "");
    onToast("❌ Account removed");
  };

  const publish = (platform) => {
    if (!selectedAccount) {
      onToast("⚠️ Please select or add a Meta account first");
      return;
    }
    const account = metaAccounts.find(a => a.id === selectedAccount);
    if (!account) {
      onToast("⚠️ Account not found");
      return;
    }
    
    if (automationMode === "auto") {
      const settings = Object.entries(autoSettings).filter(([k, v]) => v).map(([k]) => k);
      localStorage.setItem('infiCreatorAutoPost', JSON.stringify({
        accountId: selectedAccount,
        caption,
        image: postImage,
        platforms: { fb: platforms.fb, ig: platforms.ig },
        automationSettings: settings,
        createdAt: new Date().toISOString(),
      }));
      onToast(`🤖 ${platform === "fb" ? "Facebook & Instagram" : "Instagram"} automation enabled! AI will post regularly.`);
    } else {
    ...
    <div>
      ...
      <div className="pub-layout">
        ...
        <div>
          ...
          <div className="card" style={{ marginBottom: '14px', background: 'rgba(16,185,129,.1)', border: '0.5px solid rgba(16,185,129,.3)' }}>
            <div style={{ fontSize: '12px', fontWeight: '500', marginBottom: '10px' }}>🤖 Publishing Mode</div>
            <div style={{ display: 'flex', gap: '8px' }}>
              <button
                ...
                style={{
                  flex: 1,
                  padding: '8px',
                  border: automationMode === "manual" ? '0.5px solid rgba(16,185,129,.8)' : '0.5px solid var(--bdr)',
                  background: automationMode === "manual" ? 'rgba(16,185,129,.2)' : 'transparent',
              ...
              <button
                ...
                style={{
                  flex: 1,
                  padding: '8px',
                  border: automationMode === "auto" ? '0.5px solid rgba(124,58,237,.8)' : '0.5px solid var(--bdr)',
                  background: automationMode === "auto" ? 'rgba(124,58,237,.2)' : 'transparent',
                  borderRadius: 'var(--rxs)',
                  color: 'var(--txt)',
                  cursor: 'pointer',
                  fontSize: '11px',
          ...
          <div className="form-group">
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <select className="form-input form-select" style={{ flex: 1 }} value={selectedAccount} onChange={(e) => setSelectedAccount(e.target.value)}>
                <option value="">Select account...</option>
                {metaAccounts.map(acc => (
                  <option key={acc.id} value={acc.id}>{acc.icon} {acc.name} ({acc.page})</option>
                ))}
                <option value="__add__">+ Add Manual Account</option>
              </select>
              {selectedAccount && (
                <button 
            ...
            {metaAccounts.length === 0 && (
              <div style={{ fontSize: '11px', color: 'var(--txt3)', marginTop: '6px', background: 'rgba(239,68,68,.1)', padding: '8px', borderRadius: 'var(--rxs)' }}>
                ⚠️ No Meta accounts connected. Add one manually to publish.
              </div>
            )}
          </div>

          {showManualAdd && (
            <div className="card" style={{ marginBottom: '14px', background: 'rgba(124,58,237,.1)', border: '0.5px solid rgba(124,58,237,.3)' }}>
              <div style={{ fontSize: '12px', fontWeight: '500', marginBottom: '10px' }}>🔑 Add Meta Account Manually</div>
              <div className="form-group">
                <label className="form-label">Page ID</label>
                <input 
                  className="form-input" 
                  placeholder="e.g., 122117527778991774"
                  value={manualPageId}
                  onChange={(e) => setManualPageId(e.target.value)}
                />
                <div style={{ fontSize: '10px', color: 'var(--txt2)', marginTop: '4px' }}>Find your Page ID: facebook.com/[page-name]/settings → Page Info</div>
              </div>
              <div className="form-group">
                ...
                />
                <div style={{ fontSize: '10px', color: 'var(--txt2)', marginTop: '4px' }}>Get token: https://developers.facebook.com/tools/explorer/ → Generate Access Token</div>
              </div>
              <div style={{ display: 'flex', gap: '8px' }}>
                <button className="btn btn-meta" style={{ flex: 1, justifyContent: 'center' }} onClick={addManualAccount}>
                  ✅ Add Account
                </button>
                <button className="btn btn-secondary" style={{ flex: 1, justifyContent: 'center' }} onClick={() => setShowManualAdd(false)}>
                  Cancel
                </button>
              </div>
            </div>
          ...
          {!showManualAdd && metaAccounts.length > 0 && (
            <button 
              className="btn btn-secondary" 
              style={{ width: '100%', marginBottom: '14px', justifyContent: 'center', fontSize: '12px' }}
          ...
          {!showManualAdd && metaAccounts.length === 0 && (
            <button 
              ...
              onClick={() => setShowManualAdd(true)}
            >
              ➕ Add Meta Account Manually
            </button>
          )}

          {/* Platform Selection */}
          <div className="form-group">
            <label className="form-label">Post to Platforms</label>
            <div style={{ display: 'flex', gap: '8px' }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: '6px', flex: 1, padding: '8px', background: 'var(--surf2)', borderRadius: 'var(--rxs)', cursor: 'pointer' }}>
                <input type="checkbox" checked={platforms.fb} onChange={(e) => setPlatforms({ ...platforms, fb: e.target.checked })} />
                📘 Facebook
              </label>
              <label style={{ display: 'flex', alignItems: 'center', gap: '6px', flex: 1, padding: '8px', background: 'var(--surf2)', borderRadius: 'var(--rxs)', cursor: 'pointer' }}>
                <input type="checkbox" checked={platforms.ig} onChange={(e) => setPlatforms({ ...platforms, ig: e.target.checked })} />
                📸 Instagram
              </label>
            </div>
          </div>
          ...
          {automationMode === "auto" && (
            <div className="card" style={{ marginBottom: '14px', background: 'rgba(124,58,237,.08)', border: '0.5px solid rgba(124,58,237,.25)' }}>
              <div style={{ fontSize: '11px', color: 'var(--txt2)', marginBottom: '8px' }}>
                ⏰ Automation Schedule
              </div>
              <select className="form-input form-select">
                <option>📅 Daily (best for engagement)</option>
          ...
          <div style={{ display:"flex", gap:8 }}>
            <button 
              ...
              onClick={() => publish("fb")}
            >
              {automationMode === "auto" ? "🤖 Enable FB Automation" : "📘 Publish to Facebook"}
            </button>
            <button 
              ...
              onClick={() => publish("ig")}
            >
              {automationMode === "auto" ? "🤖 Enable IG Automation" : "📸 Post to Instagram"}
            </button>
          </div>
          <button 
            className="btn btn-secondary" 
            style={{ width:"100%", justifyContent:"center", marginTop:7 }} 
            onClick={() => onToast(automationMode === "auto" ? "✅ AI Automation enabled! Posts will go out automatically." : "⏰ Post scheduled — AI will publish at optimal time")}
          >
            {automationMode === "auto" ? "🤖 Start AI Automation" : "⏰ Schedule with AI"}
          </button>
        </div>
        {/* Previews */}
        <div>
          <div className="plat-switch">
            {platforms.fb && <button className={`psw-btn fb ${previewPlatform === "fb" ? "active" : ""}`} onClick={() => setPreviewPlatform("fb")}>📘 Facebook</button>}
            {platforms.ig && <button className={`psw-btn ig ${previewPlatform === "ig" ? "active" : ""}`} onClick={() => setPreviewPlatform("ig")}>📸 Instagram</button>}
          </div>
          {previewPlatform === "fb"
...
function CampaignsPanel({ onToast }) {
  ...
  const addManualAccount = () => {
    ...
    setMetaAccounts([...metaAccounts, { id: Date.now(), name: "Manual", page: manualPageId, icon: "📘", token: manualToken }]);
    setManualPageId("");
    setManualToken("");
    setShowManualAdd(false);
    onToast("✅ Account added successfully!");
  };
  ...
  const publish = (platform) => {
    if (!selectedAccount) {
      onToast("❌ Please select an account first");
      return;
    }
    if (!caption && !postImage) {
      onToast("❌ Please add a caption or image");
      return;
    }
    onToast(`✅ Publishing to ${platform === "fb" ? "Facebook" : "Instagram"}${automationMode === "auto" ? " (AI-controlled)" : ""}...`);
  };
          ...
          <div className="card" style={{ marginBottom: '14px', background: 'rgba(124,58,237,.1)', border: '0.5px solid rgba(124,58,237,.3)' }}>
              ...
              <div className="form-group">
                ...
                <input 
                  ...
                  onChange={(e) => setManualToken(e.target.value)}
                />
                <div style={{ fontSize: '10px', color: 'var(--txt2)', marginTop: '4px' }}>Get token: Graph Explorer → Generate Access Token</div>
              </div>
              <div style={{ display: 'flex', gap: '8px' }}>
                <button className="btn btn-meta" style={{ flex: 1, justifyContent: 'center' }} onClick={addManualAccount}>
                  ✅ Add Account
                </button>
                <button className="btn btn-secondary" style={{ flex: 1, justifyContent: 'center' }} onClick={() => setSelectedAccount("")}>
                  Cancel
                </button>
              </div>
            </div>
      ...
      <div className="app">
        ...
        <div className={`sidebar ${sidebarCollapsed ? "collapsed" : ""} ${mobileSidebar ? "mobile-open" : ""}`}>
          ...
          <div className="sb-bottom">
            <AuthPanel onLogout={() => setToast("👋 Logged out successfully")} />
            <div className="sb-plan" style={{ marginTop: "12px" }}>
